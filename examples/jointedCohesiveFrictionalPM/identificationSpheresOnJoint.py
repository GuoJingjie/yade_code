...
def sphereMat(): return JCFpmMat(type=1, young=1, frictionAngle=radians(1), density=1, poisson=1, tensileStrength=1e6, cohesion=1e6,
                                 jointNormalStiffness=1, jointShearStiffness=1, jointTensileStrength=1e6, jointCohesion=1e6, jointFrictionAngle=1)
...
for i in O.interactions:
    ...
    if isinstance(O.bodies[i.id1].shape, Facet) and isinstance(O.bodies[i.id2].shape, Sphere):
        ...
        if O.bodies[i.id2].state.onJoint == False:  # particles has not yet been identified as belonging to a joint plane
            ...
            if O.bodies[i.id2].state.joint == 1 and ((O.bodies[i.id2].state.jointNormal1.cross(nRef)).norm() > 0.05):
            ...
            elif O.bodies[i.id2].state.joint == 2 and ((O.bodies[i.id2].state.jointNormal1.cross(nRef)).norm() > 0.05) and ((O.bodies[i.id2].state.jointNormal2.cross(nRef)).norm() > 0.05):
