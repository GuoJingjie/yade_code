...
bar = [0.0] * (nvoids)
initiallevel = O.bodies[flow.wallIds[flow.ymin]].state.pos[1] + \
    (O.bodies[flow.wallIds[flow.ymax]].state.pos[1] - O.bodies[flow.wallIds[flow.ymin]].state.pos[1]) / 3
...
def fluxtest():
    ...
    if error > toleranceWarning:
        print("Warning: difference between total water volume flowing through bottom wall and water loss due to air bubble generations",
