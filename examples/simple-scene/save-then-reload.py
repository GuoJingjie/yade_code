...
O.engines = [
    ...
    TriaxialStressController(maxMultiplier=1. + 2e4 / young, finalMaxMultiplier=1. + 2e3 / young,
                             thickness=0, stressMask=7, internalCompaction=True, label="triax"),
...
if not savedState:
    print("No saved state - running isotropic confinement for num_spheres=", str(table.num_spheres),
          ", compFricDegree=", str(table.compFricDegree), ", key='", str(table.key), "'")
    while 1:
        O.run(1000, True)
        unb = unbalancedForce()
    ...
    print("Saved state found - reload then proceed to deviatoric loading for num_spheres=",
          str(table.num_spheres), ", compFricDegree=", str(table.compFricDegree), ", key='", str(table.key), "'")
