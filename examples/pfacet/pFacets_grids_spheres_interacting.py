...
O.materials.append(CohFrictMat(young=0.05 * E, poisson=0.3, density=2650, frictionAngle=radians(phi),
                               normalCohesion=3e100, shearCohesion=3e100, momentRotationLaw=True, label='cohfrictMat'))
...
for i in range(0, nbL):
    for j in range(0, nbl):
        nodesBG.append(O.bodies.append(gridNode([(i * L / nbL) + 0.005, -0.11, j * l / nbl],
                                                r, wire=False, fixed=False, material='cohfrictMat', color=colorBG)))
# connection
