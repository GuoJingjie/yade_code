...
if size == 'large':
    ...
    z = 55.4222 / \
        1000. 	# This is the height of the lowest point of the funnel (at the orifice), measuring from the lowest cylindrical cross section of the silo
else:
    z = 59.3008 / 1000.
...
def eraseEscapedParticles():
    ...
    ers = []
    for b in O.bodies:
        # I do not delete the particles right after they pass the orifice, to disturb the simulation as little as possible
        if isinstance(b.shape, Sphere) and b.state.pos[2] < -z:
            ers.append(b.id)
            count += 1
    if mpi:
        mp.bodyErase(ers)
