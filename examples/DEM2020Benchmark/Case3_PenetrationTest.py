...
# script execution:
#   Option 1:  'yadedaily -n -x Case3_PenetrationTest.py 25000'		# use this command for efficient production (no GUI; Yade will terminate automatically when the simulation is finished)
#   Option 2:  'yadedaily Case3_PenetrationTest.py 25000'		# use this command to keep GUI (affects the overall performance)

# adapt yade executable if not 'daily' version and pick one of 25000,50000,100000
...
# -------------------------------------------------------------------- #
# Input Data - Number of particles. The rest of the script is automated for all different scenarios.
...
inputFileName = 'inputData/Case3_'+str(int(N/1000))+'KParticles.txt'
altName = 'inputData/Case3_'+str(int(N/1000))+'KParticlesSwapped.txt'
...
urls["walls"]="https://cloud.tuhh.de/index.php/s/49HZwje9zj4R78m/download"
...
sp=ymport.text(altName,material=M1) #(M1 if material=="M1" else M2)
...
wallFile = 'inputData/Case3_Walls.txt' #+str(int(N/1000))+'.txt' # Bruno: a bit pointless to download again with different N but it's to avoid collisions between parallel jobs # Vas: I think we do not need to download the same walls 3 times, in different files, even for parallel executions. If the walls exist from a previous execution, nothing goes wrong.
...
if not hasInputWall:
    ...
    try:
        os.system('wget --no-clobber -O '+wallFile+'_temp '+urls["walls"])
    except:
...
O.engines=[
	...
	InteractionLoop(
		...
		[Ip2_FrictMat_FrictMat_MindlinPhys(
			en         = MatchMaker(matches=((1,1,e_gg),(0,1,e_gs)))  # 0 being the id of Steel and 1 being the id of M1 # ,(0,0,e_ss) # e_ss is not needed (Steel-Steel), as the steel ball is not supposed to touch the steel box.
		)],
