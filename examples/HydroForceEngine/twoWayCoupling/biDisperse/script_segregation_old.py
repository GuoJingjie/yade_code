...
# 20/08/2021
#
# Same as sedimentTransportExample_1DRANSCoupling, with two grain size.
#
...
#Particles
sizeRatio = 3/2. 	#Size ratio between the two particles class radius
...
NlayerSmall = 1	#nb of layer of small particles, in large particle diameter
...
[phiPart1,averageDrag1,vxPart1,vyPart1,vzPart1,phiPart2,averageDrag2,vxPart2,vyPart2,vzPart2] = [np.zeros(ndimz),np.zeros(ndimz),np.zeros(ndimz),np.zeros(ndimz),np.zeros(ndimz),np.zeros(ndimz),np.zeros(ndimz),np.zeros(ndimz),np.zeros(ndimz),np.zeros(ndimz)] #Segregation parameter
...
#Particles diameter properties
diameterPart1 = diameterPart 
diameterPart2 = diameterPart/sizeRatio
...
partVolume1 = pi/6.*pow(diameterPart1,3) #Volume of a particle
Nlayer1 = Nlayer - NlayerSmall
partNumber1 = int(Nlayer1*phiPartMax*diameterPart1*length*width/partVolume1) #Volume of beads to obtain Nlayer layers of particles
partCloud.makeCloud(minCorner=(0,0.,groundPosition+diameterPart),maxCorner=(length,width,groundPosition+fluidHeight),rRelFuzz=0., rMean=diameterPart1/2.0, num = partNumber1)
partCloud.toSimulation(material='Mat') #Send this packing to simulation with material Mat
#Deposit small particles on top of it
partCloud = pack.SpherePack()
partVolume2 = pi/6.*pow(diameterPart2,3) #Volume of a particle
partNumber2 = int(NlayerSmall*phiPartMax*diameterPart*length*width/partVolume2) #Volume of beads to obtain NlayerSmall layers of particles
partCloud.makeCloud(minCorner=(0,0.,groundPosition+ 2*diameterPart + fluidHeight),maxCorner=(length,width,groundPosition+ 2*diameterPart + 1.5*fluidHeight),rRelFuzz=0., rMean=diameterPart2/2.0, num = partNumber2)
partCloud.toSimulation(material='Mat') #Send this packing to simulation with material Mat
...
#Activate bi-disperse measurement
hydroEngine.compatibilityOldVersion = True
hydroEngine.twoSize = True
hydroEngine.radiusPart1 = diameterPart1/2.
hydroEngine.radiusPart2 = diameterPart2/2.
...
def measure():
	...
	global phiPart1PY, vxPart1PY, phiPart2PY, vxPart2PY
	...
	#Bidisperse quantities, idem for each class of size
	[phiPart1PY,phiPart2PY,vxPart1PY,vxPart2PY]=[np.array(hydroEngine.phiPart1),np.array(hydroEngine.phiPart2),np.array(hydroEngine.vxPart1),np.array(hydroEngine.vxPart2)]
	...
	if saveData==1:	#Save data for postprocessing
		...
		globalParam =  ['qsMean','phiPartPY','vxPartPY','vxFluidPY','zAxis','phiPart1PY','phiPart2PY','vxPart1PY','vxPart2PY']	# Variables to save
