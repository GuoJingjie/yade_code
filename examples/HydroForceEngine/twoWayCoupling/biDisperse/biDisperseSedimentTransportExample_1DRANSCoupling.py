...
maxPressure = (densPart-densFluid)*phiPartMax*(Nlayer1*diameterPart1+Nlayer2*diameterPart2)*abs(gravityVector[2]) #Estimated max particle pressure from the static load
...
WaterSurface = box(center= (length/2.0, width/2.0,groundPosition+fluidHeight),extents=(2000,width/2.0,0),fixed=True,wire=False,color = (0,0,1),material = 'Mat1',mask = 0)
...
for x in L: #loop creating a set of sphere sticked at the bottom with a (uniform) random altitude comprised between 0.5 (diameter/12) and 5.5mm (11diameter/12) with steps of 0.5mm. The repartition along z is made around groundPosition.
    for y in W:
        n =  rand.randrange(0,12,1)/12.0*diameterPart1     #Define a number between 0 and 11/12 diameter with steps of 1/12 diameter (0.5mm in the experiment)
        O.bodies.append(sphere((x*diameterPart1, y*diameterPart1,groundPosition - 11*diameterPart1/12.0/2.0 + n),diameterPart1/2.,color=(0,0,0),fixed = True,material = 'Mat1'))
...
partCloud1.makeCloud(minCorner=(0,0.,groundPosition+diameterPart1+1e-4),maxCorner=(length,width,groundPosition+depositionHeight1),rRelFuzz=0., rMean=diameterPart1/2.0, num = partNumber1)
partCloud2.makeCloud(minCorner=(0,0.,groundPosition+depositionHeight1),maxCorner=(length,width,groundPosition+depositionHeight1+depositionHeight2),rRelFuzz=0., rMean=diameterPart2/2.0, num = partNumber2)
...
O.engines = [
        ...
        HydroForceEngine(densFluid = densFluid,viscoDyn = kinematicViscoFluid*densFluid,zRef = groundPosition,gravity = gravityVector,deltaZ = dz,expoRZ = expoDrag,nCell = ndimz,vCell = length*width*dz, ids = idApplyForce, label = 'hydroEngine', phiMax=phiPartMax, dead = True),
...
def measure():
    ...
    if saveData==1: #Save data for postprocessing
        ...
        globalParam =  ['qsMean','phiPart', 'phiPart1', 'phiPart2', 'vxPart','vxPart1','vxPart2','vyPart', 'vyPart1', 'vyPart2','vzPart','vzPart1', 'vzPart2', 'vxFluid','zAxis', 'turbulentViscosity', 'turbStress']#, 'stress1', 'stress2']   # Variables to save
