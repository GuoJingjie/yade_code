...
# Add the top plate
topPlate = box(center=(length / 2.0, width / 2.0, groundPosition + fluidHeight),
               extents=(2000, 2000, 0), fixed=True, wire=False, color=(0, 1, 0), material='Mat', mask=0)
...
for x in L:
    for y in W:
        ...
        O.bodies.append(sphere((x * diameterPart, y * diameterPart, groundPosition + (-0.5 + n) * diameterPart),
                               diameterPart / 2., color=(0, 0, 0), fixed=True, material='Mat'))
...
partCloud.makeCloud(minCorner=(0, 0., groundPosition + diameterPart), maxCorner=(length, width,
                                                                                 groundPosition + fluidHeight * 1.7), rRelFuzz=0., rMean=diameterPart / 2.0, num=partNumber)
...
O.engines = [
    ...
    HydroForceEngine(densFluid=densFluidPY, viscoDyn=dynamicVisco, zRef=groundPosition, gravity=gravityVector, deltaZ=dz, expoRZ=expoDrag_PY, lift=False, nCell=ndimz, vCell=length * width * \
                     dz, radiusPart=diameterPart / 2., vxFluid=initVxFluid, phiPart=phiPartPY, vxPart=vxPartPY, irheolf=0, iusl=0, uTop=uTop, iturbu=0, ids=idApplyForce, label='hydroEngine', dead=True),
...
def gravityDeposition(lim):
    if O.time < lim:
        ...
        hydroEngine.ReynoldStresses = np.zeros(ndimz)
