...
for x in L:
    for y in W:
        ...
        O.bodies.append(sphere((x * diameterPart, y * diameterPart, groundPosition - 11 * diameterPart /
...
partCloud.makeCloud(minCorner=(0, 0., groundPosition + diameterPart), maxCorner=(length, width,
                                                                                 groundPosition + fluidHeight), rRelFuzz=0., rMean=diameterPart / 2.0, num=partNumber)
...
O.engines = [
    ...
    HydroForceEngine(densFluid=densFluidPY, viscoDyn=kinematicViscoFluid * densFluidPY, zRef=groundPosition, gravity=gravityVector, deltaZ=dz, expoRZ=expoDrag_PY,
                     nCell=ndimz, vCell=length * width * dz, vxFluid=vxFluidPY, phiPart=phiPartPY, vxPart=vxPartPY, ids=idApplyForce, label='hydroEngine', dead=True),
...
def gravityDeposition(lim):
    if O.time < lim:
        ...
        hydroEngine.simplifiedReynoldStresses = np.ones(ndimz) * 1e-4
