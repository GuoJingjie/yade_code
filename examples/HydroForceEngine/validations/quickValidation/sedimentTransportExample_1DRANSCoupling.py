...
lengthCell = 8	#Streamwise length of the periodic cell, in diameter
...
O.engines = [
	...
	HydroForceEngine(densFluid = densFluidPY,viscoDyn = kinematicViscoFluid*densFluidPY,zRef = groundPosition,gravity = gravityVector,deltaZ = dz,expoRZ = expoDrag_PY,lift = False,nCell = ndimz,vCell = length*width*dz,radiusPart=diameterPart/2.,ids = idApplyForce, label = 'hydroEngine', dead = True, compatibilityOldVersion = False),
...
#		print(np.array(hydroEngine.vPart)[:,0])
#		hydroEngine.vxPart = np.array(hydroEngine.vPart)[:,0]
#		print(np.array(hydroEngine.phiPart))
