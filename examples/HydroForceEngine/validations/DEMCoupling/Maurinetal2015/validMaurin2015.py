...
WaterSurface = box(center=(length / 2.0, width / 2.0, groundPosition + fluidHeight), extents=(2000,
                                                                                              width / 2.0, 0), fixed=True, wire=False, color=(0, 0, 1), material='Mat', mask=0)
...
# loop creating a set of sphere sticked at the bottom with a (uniform) random altitude comprised between 0.5 (diameter/12) and 5.5mm (11diameter/12) with steps of 0.5mm. The repartition along z is made around groundPosition.
for x in L:
    for y in W:
        ...
        O.bodies.append(sphere((x * diameterPart, centerLimitY, groundPosition - 11 * diameterPart /
                                12.0 / 2.0 + n), diameterPart / 2., color=(0, 0, 0), fixed=True, material='Mat'))
...
partCloud.makeCloud(minCorner=(0, centerLimitY, groundPosition + diameterPart), maxCorner=(length, centerLimitY,
                                                                                           groundPosition + fluidHeight * 2), rRelFuzz=0., rMean=diameterPart / 2.0, num=partNumber)
...
O.engines = [
    ...
    HydroForceEngine(densFluid=densFluidPY, viscoDyn=kinematicViscoFluid * densFluidPY, zRef=groundPosition, gravity=gravityVector, deltaZ=dz, expoRZ=expoDrag_PY, lift=False, nCell=ndimz, vCell=length * width * dz, radiusPart=diameterPart / \
                     2., vxFluid=np.array(vxFluidPY), phiPart=phiPartPY, vxPart=vxPartPY, ids=idApplyForce, label='hydroEngine', dead=True, fluidWallFriction=True, channelWidth=width, phiMax=phiPartMax, iturbu=1, ilm=2, iusl=1, irheolf=0),
...
def gravityDeposition(lim):
    if O.time < lim:
        ...
        hydroEngine.ReynoldStresses = np.ones(ndimz) * 1e-4
