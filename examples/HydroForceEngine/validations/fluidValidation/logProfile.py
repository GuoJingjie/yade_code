...
a = HydroForceEngine(densFluid=densFluid, viscoDyn=kinematicViscoFluid * densFluid, deltaZ=dz, nCell=ndimz, vxFluid=np.array(vxFluid), gravity=gravityVector, phiMax=0.61, turbulentViscosity=np.zeros(ndimz), iturbu=1,
                     ilm=0, iusl=1, viscousSubLayer=1, phiPart=np.zeros(ndimz), vxPart=np.zeros(ndimz), averageDrag=np.zeros(ndimz), kappa=0.41)  # Define the minimum parameters of the HydroForceEngine for the fluid resolution
