...
def collectTiming(Cundall):
    ...
    f.write(str(Cundall) + " " + str(numThreads * int(os.getenv('OMP_NUM_THREADS'))) + " " + str(os.getenv('OMPI_COMM_WORLD_SIZE')) + " " +
            os.getenv('OMP_NUM_THREADS') + " " + str(N * M * L * (numThreads - 1)) + " " + str(L) + " " + str(M) + " " + str(N) + " " + str(timing.runtime()) + "\n")
    f.close()
