...
if 1:
    ...
    globalDomain = dd.Globaldomain(mn, mx, xDecomp=numThreads - 1, yDecomp=1, zDecomp=1)
    # sequential grain colors
#	import colorsys
#	colorScale = (Vector3(colorsys.hsv_to_rgb(value*1.0/numThreads, 1, 1)) for value in range(0, numThreads))
    colors = []
...
def collectTimingMPI():
    ...
    ) / 1e9, isendRecvForcesRunner.execTime / 1e9, sendRecvStatesRunner.execTime / 1e9, waitForcesRunner.execTime / 1e9, collisionChecker.execTime / 1e9, collider.execTime / 1e9))
    f.close()
...
def collectTiming():
    ...
    from yade import timing
    f.write(str(numThreads) + " " + str(os.getenv('OMPI_COMM_WORLD_SIZE')) + " " + os.getenv('OMP_NUM_THREADS') +
            " " + str(N * M * (numThreads - 1)) + " " + str(N) + " " + str(M) + " " + str(timing.runtime()) + "\n")
    f.close()
