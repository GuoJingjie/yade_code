...
if rank == 0:
    ...
    globalDomain = dd.Globaldomain(mn, mx, xDecomp=1, yDecomp=1, zDecomp=numThreads - 1)
...
def collectTimingMPI():
    ...
    f.write("%s %s %s %g %g %g %g %g %g %g %g\n" % (numThreads, str(os.getenv('OMPI_COMM_WORLD_SIZE')), os.getenv('OMP_NUM_THREADS'), len(O.bodies), O.forces.f(WALL_ID)[1], timing.runtime(
    ) / 1e6, isendRecvForcesRunner.execTime / 1e6, sendRecvStatesRunner.execTime / 1e6, waitForcesRunner.execTime / 1e6, collisionChecker.execTime / 1e6, collider.execTime / 1e6))
    f.close()
...
def collectTiming():
    ...
    from yade import timing
    f.write(str(globalDomain.numThreads) + " " + str(os.getenv('OMPI_COMM_WORLD_SIZE')) + " " + os.getenv('OMP_NUM_THREADS') +
            " " + str(len(O.bodies)) + " " + str(timing.runtime()) + " " + str(O.forces.f(WALL_ID)[1]) + "\n")
    f.close()
