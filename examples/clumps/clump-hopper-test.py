...
def addBodies():
    ...
    for k, l in itertools.product(arange(0, 10), arange(0, 10)):
        clpId, sphId = O.bodies.appendClumped([sphere(Vector3(x0t + Rs * (k * 4 + 2), y0t + Rs * (l * 4 + 2), i * Rs *
                                                              2 + zt + ab * 3), Rs, color=clumpColor, material=dfltSpheresMat) for i in range(4)])
        numSphereGen += len(sphId)

    # ... and spheres
    spheresColor = (0.4, 0.4, 0.4)
    for k, l in itertools.product(arange(0, 9), arange(0, 9)):
        sphAloneId = O.bodies.append([sphere(Vector3(x0t + Rs * (k * 4 + 4), y0t + Rs * (l * 4 + 4), i * Rs * 2.3 +
                                                     zt + ab * 3), Rs, color=spheresColor, material=dfltSpheresMat) for i in range(4)])
        numSphereGen += len(sphAloneId)


def state():
    global numSphereGen
    print("Iter %d: Total number of generated spheres %d, removed particles %d, current particles %d, kinEnergy %g" %
