...
# mean radius and relative fuzz of the radius (random, uniformly distributed between sphRad*(1-.5*sphRadFuzz)…sphRad*(1+.5*sphRadFuzz))
sphRad, sphRadFuzz = 0.03, .8
...
mrs2 = millRad * sqrt(2)
cap1, cap2 = [Vector3(0, 0, mrs2), Vector3(0, -mrs2, 0), Vector3(0, 0, -mrs2)], [Vector3(0, 0, mrs2),
                                                                                 Vector3(0, 0, -mrs2), Vector3(0, mrs2, 0)]  # 2 triangles at every side
for xx in -.5 * millDp, .5 * millDp:
    millIds += O.bodies.append([facet([p + Vector3(xx, 0, 0) for p in cap1], color=(0, 0, 0)), facet([p + Vector3(xx, 0, 0) for p in cap2], color=(0, 0, 0))])

# define domains for initial cloud of red and blue spheres
packHt = .8 * millRad  # size of the area
bboxes = [(Vector3(-.5 * millDp, -.5 * packHt, -.5 * packHt), Vector3(.5 * millDp, 0, .5 * packHt)),
