...
sphs = [utils.sphere(radius=m_sphRadius, center=m_sphRadius * Vector3(x, random.uniform(-0.5, 0.5), z), fixed=True, mask=3)
        for x in range(0, sq + 2) for z in range(0, sq + 2)]
...
O.engines = [ForceResetter(),
    ...
    # Maybe important: apply drag for particles with less than 2 interactions: prevent free fall on the interface
    PyRunner(command="drag.ids=[b.id for b in O.bodies if len(O.interactions.withBody(b.id)) < 2 and b.mask == 2]", iterPeriod=1000),
