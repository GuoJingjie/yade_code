...
def yadesrc_role(role, rawtext, lineno, inliner, options={}, content=[]):
    ...
    if m:
        txt, id = m.group(1), m.group(2)
    # **options should be passed to nodes.reference as well
    return [nodes.reference(rawtext, docutils.utils.unescape(txt), refuri='https://gitlab.com/yade-dev/trunk/blob/master/%s' % id)], []
...
def yadesrccommit_role(role, rawtext, lineno, inliner, options={}, content=[]):
    ...
    if m:
        txt, id = m.group(1), m.group(2)
    # **options should be passed to nodes.reference as well
    return [nodes.reference(rawtext, docutils.utils.unescape(txt), refuri='https://gitlab.com/yade-dev/trunk/blob/%s' % id)], []
...
moduleMap = {
    'yade._libVersions': 'yade.libVersions', 'yade._log': 'yade.log', 'yade._math': 'yade.math', 'yade._minieigenHP': 'yade.minieigenHP', 'yade._packPredicates': 'yade.pack', 'yade._packSpheres': 'yade.pack', 'yade._packObb': 'yade.pack', 'yade._utils': 'yade.utils', 'yade._polyhedra_utils': 'yade.polyhedra_utils', 'yade.qt._GLViewer': 'yade.qt'
}
