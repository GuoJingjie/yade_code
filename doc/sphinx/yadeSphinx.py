...
mods = {
    'libVersions': ['_libVersions'], 'log': ['_log'], 'math': ['_math'], 'minieigenHP': ['_minieigenHP'], 'export': [], 'post2d': [], 'pack': ['_packSpheres', '_packPredicates', '_packObb'], 'plot': [], 'timing': [], 'utils': ['_utils'], 'polyhedra_utils': ['_polyhedra_utils'], 'ymport': [], 'geom': [], 'bodiesHandling': [], 'qt': ['qt._GLViewer'], 'linterpolation': [], 'gridpfacet': [], 'mpy': []
}
#
    ...
    for l in open(f, encoding="utf8"):
        if writer == 'html':
            ...
            if((not (r'\pysiglinewithargsret{\sphinxstrong{class }\sphinxcode{yade.wrapper.}\sphinxbfcode{' in l and r'\emph{inherits' in l))  # debian stretch, devuan ascii
                    ...
                    # ubuntu bionic 18.04
                    and (not (r'\pysiglinewithargsret{\sphinxbfcode{class }\sphinxcode{yade.wrapper.}\sphinxbfcode{' in l and r'\emph{inherits' in l))
               # debian buster, ubuntu cosmic 18.10, ubuntu disco, debian sid
               and (not (r'\pysiglinewithargsret{\sphinxbfcode{\sphinxupquote{class }}\sphinxcode{\sphinxupquote{yade.wrapper.}}\sphinxbfcode{\sphinxupquote{' in l and r'\emph{inherits' in l))
               ):
                out.append(l)
                continue
            # print l
