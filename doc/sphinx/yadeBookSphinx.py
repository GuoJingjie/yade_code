...
def inheritanceDiagram(klass):
	...
	def mkNode(c, style='solid', fillcolor=None):
		return '\t\t"%s" [shape="box",fontsize=8,style="setlinewidth(0.5),%s",%sheight=0.2,URL="yade.wrapper.html#yade.wrapper.%s"];\n' % (
		...
		if (writer == 'latex'):
			for texdoc in texDocs:
				...
				for i in range(0, len(lines)):
					if (
					        i != len(lines) and lines[i].strip() == "\\begin{Verbatim}[commandchars=\\\\\{\\}]" and
