...
print('                      ' + str(thresholdDict['view']).ljust(11) + str(thresholdDict['term']
                                                                            ).ljust(11) + str(thresholdDict['cont']).ljust(11) + str(thresholdDict['insp']).ljust(11))
print('                      ------------------------------------------')
...
try:
        ...
        if ref.shape == scr.shape:
            ...
            while j < len(refParts):
                if(refParts[j][0].shape == scrParts[j][0].shape):
                    ...
                    print(('\033[91m' if nowHasError else '') + str(round(error, 6)).ljust(11) +
                          ('\033[0m' if nowHasError else ''), end=('' if (j != 3 or rowHadError) else '\n'))
                    if(nowHasError):
    ...
    print('                      ------------------------------------------')
    print('maximum error found   ' + str(round(maxEncountered['view'], 6)).ljust(11) + str(round(maxEncountered['term'], 6)
                                                                                           ).ljust(11) + str(round(maxEncountered['cont'], 6)).ljust(11) + str(round(maxEncountered['insp'], 6)).ljust(11))
