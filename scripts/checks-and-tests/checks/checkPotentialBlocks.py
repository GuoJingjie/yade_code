...
if ('POTENTIAL_BLOCKS' in features):
    ...
    for b in O.bodies:
        ...
        if vertices_sum != len(b.shape.vertices) or vertices_sum != 8:
            errMsg += str("Wrong vertices for body id=" + str(b.shape.id) + ", vertices_sum=" +
                          str(vertices_sum) + " len(b.shape.vertices)=" + str(len(b.shape.vertices)) + "\n")
            errors += 1
    ...
    for b in O.bodies:
        ...
        if (abs(b.shape.inertia[0] - target) / abs(target) > tol or
            ...
            errMsg += str("Wrong inertia for O.bodies[" + str(b.shape.id) + "]: " + str(b.shape.inertia) +
                          " , instead of: " + str(Vector3(target, target, target)) + "\n")
            errors += 1
