...
O.engines = [
    ...
    InteractionLoop(
        ...
        [Law2_ScGeom_JCFpmPhys_JointedCohesiveFrictionalPM(smoothJoint=False, neverErase=True, recordCracks=True,
                                                           recordMoments=True), Law2_ScGeom_FrictPhys_CundallStrack()],
    ),
    NewtonIntegrator(damping=0.7), UniaxialStrainer(strainRate=-0.5, axis=axis, asymmetry=0, posIds=posIds, negIds=negIds,
                                                    crossSectionArea=crossSectionArea, blockDisplacements=False, blockRotations=False, setSpeeds=True)
]
...
if(yade.math.getDigits10(1) == 15):
    if not len(cracksdata) == sum(AEdata[:, 5]):  # number of cracks should be equivalent to sum of cluster counts
        raise YadeCheckError('JCFpm checktest: cracks or AE clustering algorithm incorrect: len(cracksdata)=' +
                             str(len(cracksdata)) + ', sum(AEdata[:,5])=' + str(sum(AEdata[:, 5])))
else:
    # no idea why on higher precision this nummber differs a little; See https://gitlab.com/yade-dev/trunk/-/issues/174 for details.
    if (abs(len(cracksdata) - sum(AEdata[:, 5])) > 1):
        raise YadeCheckError('JCFpm checktest: cracks or AE clustering algorithm incorrect: len(cracksdata)=' +
                             str(len(cracksdata)) + ', sum(AEdata[:,5])=' + str(sum(AEdata[:, 5])))
