...
tolerance = 0.01
interactive = False
tt = TriaxialTest(internalCompaction=True, numberOfGrains=100, compactionFrictionDeg=0,
                  sphereFrictionDeg=30, importFilename=checksPath + '/data/checkTestTriax.spheres')
tt.generate("checkTest.yade")
O.load("checkTest.yade")
O.run(2020, True)
if interactive:
    print(O.engines[4].stress(0)[0], O.engines[4].stress(1)[0], O.engines[4].stress(2)[1],
          O.engines[4].stress(3)[1], O.engines[4].stress(4)[2], O.engines[4].stress(5)[2])
    os.system('gnuplot -e "plot \'./WallStresses\' using 1:3; replot' + checksPath + '\'/data/WallStressesCheckTest\' using 1:3; replot ' +
              checksPath + '\'/data/WallStresses\' using 1:4; replot ' + checksPath + '\'/data/WallStressesCheckTest\' using 1:4; pause -1"')
