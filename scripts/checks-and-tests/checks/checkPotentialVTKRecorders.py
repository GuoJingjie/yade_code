...
if((opts.threads != None and opts.threads != 1) or (opts.cores != None and opts.cores != '1')):
    raise YadeCheckError("This test will only work on single core, because it must be fully reproducible, but -j " +
                         str(opts.threads) + " or --cores " + str(opts.cores) + " is used.")
...
#						print("checking section: ", section, ("(non-matching lines allowed)" if (section in toSkip) else ""))
                        ...
                        raise YadeCheckError("checkPotentialVTKRecorders cannot determine section name in file " + fname +
                                             " line: " + str(lineCount) + " with lines: \n" + line1 + "\nvs.\n" + line2)
                ...
                if((line1 != line2) and (not isHeader)):  # we have some differences, check if they are acceptable
                    ...
                    if(section in toSkip):
                        ...
                        if(len(sp1) != len(sp2)):
                            raise YadeCheckError("checkPotentialVTKRecorders failed in file " + fname + " line: " + str(lineCount) +
                                                 ", because the lines have different elements:\n" + line1 + "\nvs.\n" + line2)
                        for s1, s2 in zip(sp1, sp2):
                            try:
                                if(abs(float(s1) - float(s2)) > 5e-6):
                                    raise YadeCheckError("checkPotentialVTKRecorders failed float comparison in file " + fname +
                                ...
                                if(s1 == '>' and s2 == '/>'):
                                    ...
                                    raise YadeCheckError("checkPotentialVTKRecorders failed string comparison in file " + fname +
                                                         " line: " + str(lineCount) + " with inputs: '" + str(s1) + "' vs. '" + str(s2) + "'")
...
if ('POTENTIAL_PARTICLES' in features) and ('VTK' in features):
    ...
    O.engines = [
        ...
        PotentialParticleVTKRecorder(fileName=vtkSaveDir + 'pp', firstIterRun=10, iterPeriod=2000, twoDimension=False, sampleX=10, sampleY=10,
                                     sampleZ=10, maxDimension=0.2, REC_INTERACTION=True, REC_COLORS=True, REC_VELOCITY=True, REC_ID=True, label='vtkRecorder')
    ]
...
if ('POTENTIAL_BLOCKS' in features) and ('VTK' in features):
    ...
    O.engines = [
        ...
        NewtonIntegrator(damping=0.0, exactAsphericalRot=True, gravity=[0, 0, 0]),  # Here we deactivate gravity
        PotentialBlockVTKRecorder(fileName=vtkSaveDir + 'pb', firstIterRun=10, iterPeriod=2000, twoDimension=False, sampleX=10, sampleY=10,
                                  sampleZ=10, maxDimension=0.2, REC_INTERACTION=True, REC_COLORS=True, REC_VELOCITY=True, REC_ID=True, label='vtkRecorder')
    ]
