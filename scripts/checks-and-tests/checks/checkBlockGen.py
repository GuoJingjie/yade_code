...
#	p.unitWidth2D = 9.0
    ...
    if a != average or b != average or c != average or c != average or a != 32:  # 32 faces of all bodies
        errMsg += str("Wrong calculation of faces of all the generated blocks: " + str(average) +
    ...
    tol = 1e-6
    target = Vector3.Zero
    target = Vector3(0.5 * (p.boundarySizeXmax - p.boundarySizeXmin), 0.5 * (p.boundarySizeYmax -
                                                                             p.boundarySizeYmin), 0.5 * (p.boundarySizeZmax - p.boundarySizeZmin))
    centroid = target
