...
# 100 was not enough to have reasonable number of collisions, so I put 200 spheres.
    ...
    if(vtkVer[0] not in [6, 8, 9]):
        raise YadeCheckError("checkVTKRecorder does not have reference results for VTK version " + str(vtkVer) + ", check files in " +
                             vtkSaveDir + ", if they are correct add them to: scripts/checks-and-tests/checks/data/vtk_reference_" + str(vtkVer[0]) + '/')
    if(vtkVer[0] == 6):
        ...
        for line1, line2 in zip(referenceFile, testedFile):
            ...
            if(t1[0] == '        <DataArray type=' and t2[0] == '        <DataArray type='):
                if(t1[3] == t2[3]):
                    ...
                    raise YadeCheckError("checkVTKRecorder cannot determine section name in file " + fname +
            ...
            if((line1 != line2) and (not isHeader)):  # we have some differences, check if they are acceptable
                ...
                if(section in toSkip):
                    ...
                    if(len(sp1) != len(sp2)):
                        raise YadeCheckError("checkVTKRecorder failed in file " + fname + " line: " + str(lineCount) +
                                             ", because the lines have different elements:\n" + line1 + "\nvs.\n" + line2)
                    for s1, s2 in zip(sp1, sp2):
                        try:
                            if(abs(float(s1) - float(s2)) > 1e-8):
                                raise YadeCheckError("checkVTKRecorder failed float comparison in file " + fname + " line: " +
                            ...
                            if(s1 == '>' and s2 == '/>'):
                                ...
                                raise YadeCheckError("checkVTKRecorder failed string comparison in file " + fname + " line: " +
                                                     str(lineCount) + " with inputs: '" + str(s1) + "' vs. '" + str(s2) + "'")
