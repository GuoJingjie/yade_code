...
def parseProject(proFile, inheritedVariables={}):
    ...
    for stat in qm:
        ...
        conditional = match(r'^\s*(!?[a-zA-Z0-9_]+)\s*{(.*)}$', stat)
        if conditional:
...
def scriptGen(v, dir):
    ...
    if buildEngine == 'waf':
        ...
        if (env, targetType) in installable:
            ...
            installable[(env, targetType)] = [pythonicTarget(target), ]
        if len(libs) > 0:
...
installDirs = {'shlib': join('$PREFIX', 'lib', 'yade$POSTFIX', string.split(scriptDir, os.sep)
                             [-1]), 'staticlib': join('$PREFIX', 'lib', 'yade$POSTFIX'), 'program': join('$PREFIX', 'bin')}
