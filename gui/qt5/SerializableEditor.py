...
_fundamentalEditorMap = {
    bool: AttrEditor_Bool, str: AttrEditor_Str, int: AttrEditor_Int, float: AttrEditor_Float, complex: AttrEditor_Complex, Quaternion: AttrEditor_Quaternion, Vector2: AttrEditor_Vector2, Vector3: AttrEditor_Vector3, Vector6: AttrEditor_Vector6, Matrix3: AttrEditor_Matrix3, Vector6i: AttrEditor_Vector6i, Vector3i: AttrEditor_Vector3i, Vector2i: AttrEditor_Vector2i, Se3FakeType: AttrEditor_Se3, EnumDropDownMenu: AttrEditor_QComboBox
}

_fundamentalInitValues = {
    bool: True, str: '', int: 0, float: 0.0, complex: 0.0j, Quaternion: Quaternion((0, 1, 0), 0.0), Vector3: Vector3.Zero, Matrix3: Matrix3.Zero, Vector6: Vector6.Zero, Vector6i: Vector6i.Zero, Vector3i: Vector3i.Zero, Vector2i: Vector2i.Zero, Vector2: Vector2.Zero, Se3FakeType: (Vector3.Zero, Quaternion((0, 1, 0), 0.0)), EnumDropDownMenu: 0
}
    ...
    def mkWidget(self, entry):
        ...
        if issubclass(entry.T, Serializable) or entry.T == Serializable:
            ...
            widget = SerializableEditor(getattr(self.ser, entry.name), parent=self, showType=self.showType,
                                        path=(self.path + '.' + entry.name if self.path else None))
...
def makeSerializableLabel(ser, href=False, addr=True, boldHref=True, num=-1, count=-1):
    ...
    # do not show address if there is a label already
        ...
        if m:
            ...
            logging.warning(u"Serializable converted to str ('%s') does not contain 'instance at 0x…'" % ss)
    return ret
...
class SeqSerializableComboBox(QFrame):
    def __init__(self, parent, getter, setter, serType, path=None, shrink=False):
        ...
        for w in buttons[0:2] + [self.combo, ] + buttons[2:4]:
