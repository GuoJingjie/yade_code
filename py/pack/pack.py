...
if (True):
    ...
    class inGtsSurface_py(Predicate):
        ...
        def __call__(self, _pt, pad=0.):
            ...
            pp = [gts.Point(_pt[0] - pad, _pt[1], _pt[2]), gts.Point(_pt[0] + pad, _pt[1], _pt[2]), gts.Point(_pt[0], _pt[1] - pad, _pt[2]),
                  gts.Point(_pt[0], _pt[1] + pad, _pt[2]), gts.Point(_pt[0], _pt[1], _pt[2] - pad), gts.Point(_pt[0], _pt[1], _pt[2] + pad)]
...
def regularOrtho(predicate, radius, gap, **kw):
    ...
    if(max([mx[i] - mn[i] for i in (0, 1, 2)]) == float('inf')):
        raise ValueError("Aabb of the predicate must not be infinite (didn't you use union | instead of intersection & for unbounded predicate such as notInNotch?")
...
def _memoizePacking(memoizeDb, sp, radius, rRelFuzz, wantPeri, fullDim, noPrint=False):
    ...
    c.execute('insert into packings values (?,?,?,?,?,?,?,?,?)', (radius, rRelFuzz,
                                                                  packDim[0], packDim[1], packDim[2], len(sp), time.time(), wantPeri, packBlob,))
    c.close()
    conn.commit()
    if not noPrint:
...
def _getMemoizedPacking(memoizeDb, radius, rRelFuzz, x1, y1, z1, fullDim, wantPeri, fillPeriodic, spheresInCell, memoDbg=False, noPrint=False):
    ...
    try:
        ...
        raise RuntimeError("ERROR: database `" + memoizeDb +
    ...
    for row in c:
        ...
        Y *= scale
        Z *= scale
        memoDbgMsg("Considering packing (radius=%g±%g,N=%g,dim=%g×%g×%g,%s,scale=%g), created %s" %
        ...
        if isPeri and wantPeri:
            ...
            if abs((y1 / x1) / (Y / X) - 1) > 0.3 or abs((z1 / x1) / (Z / X) - 1) > 0.3:
                memoDbgMsg("REJECT: proportions (y/x=%g, z/x=%g) differ too much from what is desired (%g, %g)." % (Y / X, Z / X, y1 / x1, z1 / x1))
                continue
        else:
        ...
        memoDbgMsg("ACCEPTED")
        if not noPrint:
            print("Found suitable packing in %s (radius=%g±%g,N=%g,dim=%g×%g×%g,%s,scale=%g), created %s" %
...
def randomDensePack(predicate, radius, material=-1, dim=None, cropLayers=0, rRelFuzz=0., spheresInCell=0, memoizeDb=None, useOBB=False, memoDbg=False, color=None, returnSpherePack=None, seed=-1):
    ...
    *O.switchScene()* magic is used to have clean simulation for TriaxialTest without deleting the original simulation.
    ...
    if wantPeri:
        ...
        O.engines = [ForceResetter(), InsertionSortCollider([Bo1_Sphere_Aabb()], verletDist=.05 * radius), InteractionLoop([Ig2_Sphere_Sphere_ScGeom()], [Ip2_FrictMat_FrictMat_FrictPhys()], [Law2_ScGeom_FrictPhys_CundallStrack()]),
                     PeriIsoCompressor(charLen=2 * radius, stresses=[-100e9, -1e8], maxUnbalanced=1e-2, doneHook='O.pause();', globalUpdateInt=5, keepProportions=True), NewtonIntegrator(damping=.6)]
...
def randomPeriPack(radius, initSize, rRelFuzz=0.0, memoizeDb=None, noPrint=False, seed=-1):
    ...
    """
    from math import pi
    sp = _getMemoizedPacking(memoizeDb, radius, rRelFuzz, initSize[0], initSize[1], initSize[2], fullDim=Vector3(
        0, 0, 0), wantPeri=True, fillPeriodic=False, spheresInCell=-1, memoDbg=True, noPrint=noPrint)
    if sp:
        return sp
    O.switchScene()
    O.resetThisScene()
    ...
    O.engines = [ForceResetter(), InsertionSortCollider([Bo1_Sphere_Aabb()], verletDist=.05 * radius), InteractionLoop([Ig2_Sphere_Sphere_ScGeom()], [Ip2_FrictMat_FrictMat_FrictPhys()], [Law2_ScGeom_FrictPhys_CundallStrack()]),
                 PeriIsoCompressor(charLen=2 * radius, stresses=[-100e9, -1e8], maxUnbalanced=1e-2, doneHook='O.pause();', globalUpdateInt=20, keepProportions=True), NewtonIntegrator(damping=.8)]
