...
class TestColliderByGiulia(unittest.TestCase):
    def setUp(self):
        self.wontFixIssue7 = [(-1., True, 2, 0, 0.), (1., False, 2, 0, 0.), (-1., True, 0, 1, 0.),
                              (1., False, 0, 1, 0.), (-1., True, 1, 2, 0.), (1., False, 1, 2, 0.)]
    ...
    def noteFail(self, sign, ordering, axis, initialSortAxis, change):
        # bacause the movement is along Z axis, while axis numbering referes to X. Better to print the axis which causes problems, than axis used in the loop numbering.
        axis = (2 - axis) % 3
                ...
                for sign in [-1.0, 1.0]:
                    for ordering in [True, False]:
                        for change in [1e-15, 0]:
                            O.reset()
                            O.periodic = True
                            O.cell.setBox(Vector3(self.cellSizes[(0 + axis) % 3] + change, self.cellSizes[(1 + axis) %
                                                                                                          3] + change, self.cellSizes[(2 + axis) % 3] + change))
                            # Big blue sphere
                            ...
                            # Red small sphere
                            centerSmall = [centerPos[0], centerPos[1] + 1. / 5. * self.DbigSphere,
