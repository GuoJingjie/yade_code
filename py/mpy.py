...
AUTO_COLOR = True
# Reduces the size of position/velocity comms (at the end of the colliding phase, we can exclude those bodies with no interactions besides body<->subdomain from intersections).
MINIMAL_INTERSECTIONS = False
    ...
    if needsCollide:
        if(COPY_MIRROR_BODIES_WHEN_COLLIDE):
            if MINIMAL_INTERSECTIONS:
                # if we have tricked intersections in previous steps we set them backto full content in order to update all positions before colliding
                if hasattr(O.subD, "fullIntersections") and O.subD.fullIntersections != None:
...
def genLocalIntersections(subdomains):
    ...
    for sdId in subdomains:
        ...
        if (ENABLE_PFACETS):
            if rank == 0:
                print("this one makes mpi inneficient when ENABLE_PFACETS, contact yade devs if you are interested in developping MPI for PFacets")
...
def sendRecvStates():
    ...
    if rank != 0:  # the master process never receive updated states (except when gathering)
        for otherDomain in O.subD.intersections[rank]:
            ...
            if not USE_CPP_MPI:
                ...
                pstates.append(comm.irecv(buf[-1], otherDomain, tag=_ID_STATE_SHAPE_))
            else:
    ...
    for k in O.subD.intersections[rank]:
        ...
        if not OPTIMIZE_COM:
            timing_comm.send("sendRecvStates", genUpdatedStates(O.subD.intersections[k]),
                             dest=k, tag=_ID_STATE_SHAPE_)  # should be non-blocking if n>2, else lock?
        else:
            if not USE_CPP_MPI:
    ...
    if not O.splittedOnce:
        ...
        if rank == 0 or DISTRIBUTED_INSERT:
            ...
            for k in range(1, numThreads):
                ...
                domainBody = Body(shape=Subdomain(ids=[b.id for b in O.bodies if b.subdomain == k]), subdomain=k)
        ...
        O.engines = O.engines[:idx + 1] + [PyRunner(iterPeriod=1, initRun=True,
                                                    command="sys.modules['yade.mpy'].sendRecvStates();  ", label="sendRecvStatesRunner")] + O.engines[idx + 1:]

        # insert force communicator before Newton
        O.engines = O.engines[:idx] + [PyRunner(iterPeriod=1, initRun=True, command="sys.modules['yade.mpy'].isendRecvForces()",
                                                label="isendRecvForcesRunner")] + O.engines[idx:]
        ...
        O.engines = O.engines + [PyRunner(iterPeriod=1, initRun=True,
                                          command="if sys.modules['yade.mpy'].checkAndCollide(): O.pause();", label="collisionChecker")]
    ...
    if rank > 0 or ERASE_REMOTE_MASTER:  # suppress external bodies from scene
        ...
        for b in O.bodies:
            if not b.bounded and b.subdomain != rank:
                # a gridNode could be needed as part of interacting facet/connection even if not overlaping a specific subdomain. Assume connections are always bounded for now, we thus only check nodes.
                connected = False
...
def reallocateBodiesToSubdomains(_filter=REALLOCATE_FILTER, blocking=True):
    ...
    if blocking:  # the filter will be applied sequentially to each other domain. It can include blocking communications in subdomain pairs
        def _functor(x): return reallocateBodiesPairWiseBlocking(_filter, x)
