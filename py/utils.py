...
def saveVars(mark='', loadNow=True, **kw):
    ...
    try:
        ...
        d = kw
    # use protocol 0 as it allows storage in utf8 for python3 and ascii for python2 (boost requirement)
    Omega().tags['pickledPythonVariablesDictionary' + mark] = pickle.dumps(d, 0)
    if loadNow:
        loadVars(mark)
...
def _commonBodySetup(b, volume, geomInertia, material, pos, noBound=False, resetState=True, dynamic=None, fixed=False, blockedDOFs='xyzXYZ'):
    ...
    if isinstance(material, int):
        ...
        raise TypeError("The 'material' argument must be None (for defaultMaterial), string (for shared material label), int (for shared material id) or Material instance.")
    # resets state (!!)
    if resetState:
...
def makeVideo(frameSpec, out, renameNotOverwrite=True, fps=24, kbps=6000, bps=None):
    ...
    if bps != None:
        warnings.warn('plot.makeVideo: bps is deprecated, use kbps instead (the significance is the same, but the name is more precise)',
    ...
    for passNo in (1, 2):
        cmd = ['mencoder', 'mf://%s' % frameSpec, '-mf', 'fps=%d' % int(fps), '-ovc', 'lavc', '-lavcopts', 'vbitrate=%d:vpass=%d:threads=%d:%s' % (
            int(kbps), passNo, O.numThreads, 'turbo' if passNo == 1 else ''), '-o', ('/dev/null' if passNo == 1 else out)]
...
This class is used by :yref:`yade.utils.readParamsFromTable`.
    ...
    def __init__(self, file):
        ...
        if not 'description' in headings:
            descs = set()
            for l in lines:
                dd = ','.join(head.replace('!', '') + '=' + ('%g' % values[head] if isinstance(values[l][head],
                                                                                               float) else str(values[l][head])) for head in bangHeads).replace("'", '').replace('"', '')
                if dd in descs:
                    dd += '__line=%d__' % l
