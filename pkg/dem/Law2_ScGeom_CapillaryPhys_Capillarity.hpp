...
public:
	...
	// clang-format off
	YADE_CLASS_BASE_DOC_ATTRS_INIT_CTOR_PY(Law2_ScGeom_CapillaryPhys_Capillarity,GlobalEngine,"This engine allows one to take into account capillary forces/effects between spheres coming from the presence of interparticular liquid bridges (menisci) at existing interactions. In order to allow capillary forces between distant spheres, it is necessary to enlarge the bounding boxes using :yref:`Bo1_Sphere_Aabb::aabbEnlargeFactor` and make the Ig2 define define distant interactions via :yref:`interactionDetectionFactor<Ig2_Sphere_Sphere_ScGeom::interactionDetectionFactor>`. See also :yref:`createDistantMeniscii<Law2_ScGeom_CapillaryPhys_Capillarity.createDistantMeniscii>`. It is also necessary to disable interactions removal by the constitutive law (:yref:`Law2<Law2_ScGeom_FrictPhys_CundallStrack::neverErase>=True`). The only combinations of laws supported are currently capillary law + :yref:`Law2_ScGeom_FrictPhys_CundallStrack` and capillary law + :yref:`Law2_ScGeom_MindlinPhys_Mindlin` (and the other variants of Hertz-Mindlin)\n\nThe control parameter for simulating liquid bridges is the :yref:`capillary pressure<Law2_ScGeom_CapillaryPhys_Capillarity::capillaryPressure>` (or suction) Uc = Ugas - Uliquid. Liquid bridges properties (volume V, extent over interacting grains delta1 and delta2) are computed as a result of the defined capillary pressure and of the interacting geometry (spheres radii and interparticular distance). For this purpose, ascii files M(r=i) with i=R1/R2 (and a possible :yref:`suffix<Law2_ScGeom_CapillaryPhys_Capillarity.suffCapFiles>`) are required, containing a set of results from the resolution of the Laplace-Young equation for different configurations of the interacting geometry. Corresponding files can be downloaded from `yade-data/capillaryFiles <https://gitlab.com/yade-dev/yade-data/-/tree/main/capillaryFiles>`_ (`direct download <https://gitlab.com/yade-dev/yade-data/-/archive/main/yade-data-main.zip?path=capillaryFiles>`_) for the case of a null wetting angle or regenerated, possibly for other conditions as per point II. in :ysrc:`examples/capillaryLaplaceYoung/README.txt`.\n\nSee :ysrc:`examples/capillaryLaplaceYoung/` folder for example scripts.\n\nReferences: in english [Scholtes2009b]_ [Duriez2017b]_; more detailed, but in french [Scholtes2009d]_.",
	...
	((bool,createDistantMeniscii,false,,"For generating meniscii between distant spheres (including the case of strict contact) if true. Else only maintain the existing ones. For modeling a wetting path this flag should always be false. For a drying path it should be true for one step (initialization) then false, as in the logic of [Scholtes2009c]_"))
