...
class FrictMatCDM: public FrictMat{
	public:
	virtual ~FrictMatCDM() {};
	YADE_CLASS_BASE_DOC_ATTRS_CTOR(FrictMatCDM,FrictMat,"Material to be used for extended Hertz-Mindlin contact law. Normal direction:  parameters for Conical Damage Model  (Harkness et al. 2016, Suhr & Six 2017). Tangential direction: parameters for stress dependent interparticle friction coefficient (Suhr & Six 2016). Both models can be switched on/off separately. ",
...
class MindlinPhysCDM: public MindlinPhys
{
	...
	YADE_CLASS_BASE_DOC_ATTRS_CTOR(MindlinPhysCDM,MindlinPhys,"Representation of an interaction of an extended  Hertz-Mindlin type. Normal direction:  parameters for Conical Damage Model  (Harkness et al. 2016, Suhr & Six 2017). Tangential direction: parameters for stress dependent interparticle friction coefficient (Suhr & Six 2016). Both models can be switched on/off separately, see FrictMatCDM.",
		//((Real,tangensOfFrictionAngle,NaN,,"tan of angle of friction"))
		((Real,E,0.0,," [Pa] equiv. Young's modulus"))
		((Real,G,0.0,," [Pa] equiv. shear modulus"))
		...
		((Real,R,0.0,,"[m] contact radius in conical damage model"))
		((bool,isYielding,false,,"bool: is contact currently yielding?"))
...
class Ip2_FrictMatCDM_FrictMatCDM_MindlinPhysCDM: public IPhysFunctor{
	...
	YADE_CLASS_BASE_DOC_ATTRS(Ip2_FrictMatCDM_FrictMatCDM_MindlinPhysCDM,IPhysFunctor,"Create a :yref:`MindlinPhysCDM` from two :yref:`FrictMatCDMsExts<FrictMatCDMExt>`. ",
...
class Ip2_FrictMat_FrictMatCDM_MindlinPhysCDM: public IPhysFunctor{
	...
	YADE_CLASS_BASE_DOC_ATTRS(Ip2_FrictMat_FrictMatCDM_MindlinPhysCDM,IPhysFunctor,"Create a :yref:`MindlinPhysCDM` from one FrictMat and one FrictMatCDM instance. ",
		((shared_ptr<MatchMaker>,frictAngle,,,"Instance of :yref:`MatchMaker` determining how to compute interaction's friction angle. If ``None``, minimum value is used."))
	);
	DECLARE_LOGGER;
};
...
class Law2_ScGeom_MindlinPhysCDM_HertzMindlinCDM: public LawFunctor{
	public:
		...
		YADE_CLASS_BASE_DOC_ATTRS_DEPREC_INIT_CTOR_PY(Law2_ScGeom_MindlinPhysCDM_HertzMindlinCDM,LawFunctor,"Hertz-Mindlin model extended: Normal direction: conical damage model from Harkness et al. 2016./ Suhr & Six 2017. Tangential direction: stress dependent interparticle friction coefficient, Suhr & Six 2016. Both models can be switched on/off separately. In this version there is NO damping (neither viscous nor linear), NO adhesion and NO calc_energy, NO includeMoment, NO preventGranularRatcheting. NOT tested for periodic simulations.",
			...
			, /* ctor */
			, /* py */
			.def("ratioSlidingContacts",&Law2_ScGeom_MindlinPhysCDM_HertzMindlinCDM::ratioSlidingContacts,"Return the ratio between the number of contacts sliding to the total number at a given time.")
			.def("ratioYieldingContacts",&Law2_ScGeom_MindlinPhysCDM_HertzMindlinCDM::ratioYieldingContacts,"Return the ratio between the number of contacts yielding to the total number at a given time.")
	);
	DECLARE_LOGGER;
};
