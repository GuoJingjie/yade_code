...
#include"Ig2_Facet_Sphere_Hertz_ScGeom.hpp"
...
YADE_PLUGIN((Ig2_Facet_Sphere_Hertz_ScGeom)(Ig2_Wall_Sphere_Hertz_ScGeom));

CREATE_LOGGER(Ig2_Facet_Sphere_Hertz_ScGeom);

bool Ig2_Facet_Sphere_Hertz_ScGeom::go(const shared_ptr<Shape>& cm1,
...
//			cp = facet->vertices[(m+1>2)?0:m+1];
	...
	if (penetrationDepth>0 || c->isReal())
	{
		...
		//scm->radius1 = 2*sphereRadius;//original
		//scm->radius2 = sphereRadius;//original
		scm->radius1 = 1e8*sphereRadius;//TRY
		scm->radius2 = sphereRadius;//TRY
		if (isNew) c->geom = scm;
...
bool Ig2_Facet_Sphere_Hertz_ScGeom::goReverse(	const shared_ptr<Shape>& cm1,
								const shared_ptr<Shape>& cm2,
								const State& state1,
								const State& state2,
								const Vector3r& shift2,
...
bool Ig2_Wall_Sphere_Hertz_ScGeom::go(const shared_ptr<Shape>& cm1, const shared_ptr<Shape>& cm2, const State& state1, const State& state2, const Vector3r& shift2, const bool& force, const shared_ptr<Interaction>& c){
	...
	//ws->radius1=ws->radius2=radius; // do the same as for facet-sphere: wall's "radius" is the same as the sphere's radius //>-------------------------------ORG!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	ws->radius1=1e8*radius; //myTry
	ws->radius2=radius; // myTry
